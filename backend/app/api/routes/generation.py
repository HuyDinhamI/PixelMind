...
async def upload_image(file: UploadFile = File(...)):
    """
    Upload ảnh từ frontend và lưu tạm thời trên server
    """
    try:
...
async def generate_images(
    ...
    """
    Gửi ảnh đã upload và prompt tới Leonardo.ai API để sinh ảnh mới
    """
    try:
        # Kiểm tra session_id
        session_dir = os.path.join(TEMP_DIR, session_id)
        ...
        # Tìm file ảnh đã upload
        files = os.listdir(session_dir)
        original_image = None
        for file in files:
            if file.startswith("original_"):
                original_image = os.path.join(session_dir, file)
                break
